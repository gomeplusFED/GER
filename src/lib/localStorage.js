...
let storage = {
  ...
  setItem: InertLocalFunc(
    (...args) => {
      try {
        localStorage.setItem(
          args[0],
          callByArgs(storage.setInfo, args, storage)
        );
      } catch (e) {}
    },
    (...args) => {
  ...
  clear: InertLocalFunc(
    ...
    (key) => {
      return key
        ...
        : document.cookie.split("; ").forEach(utils.clearCookie);
    }
  ),
};

let Localstroage = (supperclass) =>
  class extends supperclass {
    ...
    setItem(errorObj) {
      ...
      storage.setItem(
        this.config.errorLSSign,
        errorObj,
        _config.validTime,
        _config.maxErrorCookieNo
      );
